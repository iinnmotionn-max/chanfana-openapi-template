...
tests/integration/     24 suites, 185 tests: seed→trade→learn→evolve; audit→
                       sweep→check-in; aether, wallet, defi, shield, growth(x),
                       buildplan, freshness, rp, local, orchestrator, command,
                       autonomy, authority-posture, ratelimit, rotation, appintegrity, callers, creator, policy, readiness
```
