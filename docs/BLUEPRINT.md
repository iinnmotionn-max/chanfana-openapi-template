...
src/engine/            The colony's brains:
  ...
  copywriter.ts      Claude writes the words; a gate rejects any figure the
                       recorded facts do not contain
...
tests/integration/     33 suites, 246 tests: seed→trade→learn→evolve; audit→
                       sweep→check-in; aether, wallet, defi, shield, growth(x),
                       buildplan, freshness, rp, local, orchestrator, command,
                       autonomy, authority-posture, ratelimit, rotation, appintegrity, callers, creator, policy, readiness, sources, dash, automation, hotpath, obsidian, learning, newsroom, copywriter
```
...
| **Newsroom deck** | cockpit panel above GROWTH | The drafts were being written and nobody could see them. Each one now shows its platform, **which writer produced it** (Claude or template), the full text on demand, and one-click Queue / Publish. The `writer_note` is the most useful line on the panel: when a Claude draft is rejected it appears in amber saying why — *the system reporting that the model tried to introduce a figure with no source and was stopped*. Publish reaches outside the system, so it carries the creator key and states plainly when it is refused. |
| **Publisher** | `src/engine/publisher.ts` · `POST /growth/post/:id/publish` | The one place a post actually leaves the system. The old path set `posted: true` right after a commented-out fetch — it claimed a post went out while nothing did, a task reported complete that never ran. Now `posted` is true **only** when a real request to a platform succeeded: the X (v2 `/2/tweets`) and LinkedIn (`/v2/ugcPosts`) adapters make real calls and report the real outcome; a non-2xx, a blocked network, or a missing extra credential all leave `posted:false` with the reason. A post that did not go out becomes `ready`, never `published` — that word is reserved for a real post. Platforms with no delivery route (instagram, blog) say so honestly rather than fake success. |
| **Copywriter gate** | `src/engine/copywriter.ts` · migration 0027 | With `ANTHROPIC_API_KEY` set, Claude writes the post copy from the same recorded facts — the model chooses every word and **not one fact**. Handing a fact set to a model buys range; it also buys the one failure mode that matters on a feed whose whole claim is "every number comes from a row": a model writing about financial performance will eventually produce a figure nobody gave it. So nothing it writes is trusted on the way out. **Every numeral in the draft must already appear in the facts** (honest roundings pass, small integers in prose are exempt, an unsourced figure is a rejection not an edit); promise language is rejected; and a returns post that fails to say *paper trading, no broker* is rejected, because that is the line a copywriter cuts first. A rejected draft falls back to the template and `posts.writer` / `posts.writer_note` record which writer produced the post and why — never a guess. Claude unlinked or unreachable is a normal path, not a skipped post. |
